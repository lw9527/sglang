...
Use the ascend backend with [memfabric_hybrid](https://gitcode.com/Ascend/memfabric_hybrid):

```bash
pip install memfabric-hybrid==1.0.5
```

Each Prefill instance self-hosts its own MemFabric config store on
`tcp://<local_ip>:<--disaggregation-bootstrap-port + 1>` and advertises the
address through the SGLang HTTP bootstrap so any Decode can discover it. The
store port is intentionally derived from the bootstrap port so users only
have a single port knob to manage. **No global `ASCEND_MF_STORE_URL` is
required**; killing one Prefill no longer breaks the rest of the fleet,
matching the GPU/Mooncake decentralized rendezvous design.
...
# MemFabric store automatically binds tcp://<local_ip>:8999 (8998 + 1).
```

For backward compatibility you may still set `ASCEND_MF_STORE_URL` to force
the legacy "every P/D shares one address" topology. This is **discouraged**
because it reintroduces the SPOF where killing the cluster's first Prefill
takes everything down:

```bash
# Legacy mode -- backward compatibility only, not recommended.
