...
| `ASCEND_MF_STORE_URL`    | **Optional, legacy.** Address of a single shared MemFabric config store during PD disaggregation. When unset (recommended), each Prefill instance self-hosts its own store at `tcp://<local_ip>:<--disaggregation-bootstrap-port + 1>` automatically (no separate flag needed) and advertises it via the SGLang HTTP bootstrap, so killing one P does not bring down the rest of the fleet. Setting this env var preserves the old "every P/D shares one address" topology and the SPOF that comes with it; kept only for backward compatibility with existing deployment scripts. |               |
