...
class MooncakeStore(HiCacheStorage, MooncakeBaseStore):
    ...
    def __init__(
        ...
        try:
            ...
            # L3 segment must be split across every rank that owns a distinct
            # shard. Under pipeline parallelism, co-located PP ranks each
            # register their own segment on the same host, so scale by
            # tp_size * pp_size -- dividing by tp_size alone over-commits the
            # segment by a factor of pp_size (#22607/#27010).
            rank_scale_factor = (
                1
